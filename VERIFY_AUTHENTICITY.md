...
1. Download Sphere_by_Horizen-1.3.0-beta.exe and Sphere_by_Horizen-1.3.0-beta.exe.asc and store the files in your Downloads folder.
...
7. Verify the installer binary:
    ...
    - Choose the Downloads\Sphere_by_Horizen-1.3.0-beta.exe file in the file dialog (the .asc signature file must reside in the same directory)
...
1. Download Sphere_by_Horizen-1.3.0-beta.exe and Sphere_by_Horizen-1.3.0-beta.exe.sha256 and store the files in your Downloads folder
2. Open a PowerShell window
3. Verify the checksums by copy/pasting the following commands:
    ```
    cd ~\Downloads
    $targetHash = (Get-Content -Path Sphere_by_Horizen-1.3.0-beta.exe.sha256 -Delimiter " ")[0].Trim()
    $fileName = (Get-Content -Path Sphere_by_Horizen-1.3.0-beta.exe.sha256 -Delimiter " ")[2].Trim()
...
4. The output of the command should equal:
    ```
    Sphere_by_Horizen-1.3.0-beta.exe: OK
    ```

## MacOS
### Verify File Authenticity
1. Download Sphere_by_Horizen-1.3.0-beta.dmg and Sphere_by_Horizen-1.3.0-beta.dmg.asc and store the files in your Downloads folder.
...
7. Verify the installer binary:
    - Right-click the Sphere_by_Horizen-1.3.0-beta.dmg file in Finder (do NOT right click on the .asc file, that will not work), then select Services -> OpenPGP: Verify Signature of File (the .asc signature file must reside in the same directory)
...
### Verify File Integrity
1. Download Sphere_by_Horizen-1.3.0-beta.dmg and Sphere_by_Horizen-1.3.0-beta.dmg.sha256 and store the files in your Downloads folder
2. Open a Terminal window
3. Verify the checksums by copy/pasting the following command:
    ```
    cd ~/Downloads \
    && shasum -a256 -c Sphere_by_Horizen-1.3.0-beta.dmg.sha256
    
    ```
4. The output of the command should equal:
    ```
    Sphere_by_Horizen-1.3.0-beta.dmg: OK
    ```

## Linux
### Verify File Authenticity
1. Download Sphere_by_Horizen-1.3.0-beta.deb and Sphere_by_Horizen-1.3.0-beta.deb.asc and store the files in your Downloads folder for Debian based systems, or Sphere_by_Horizen-1.3.0-beta.AppImage and Sphere_by_Horizen-1.3.0-beta.AppImage.asc and store the files in your Downloads folder for all other Linux systems.
...
6. Verify the installer binary using the .asc signature (the .asc signature file must reside in the same directory as the installer binary):
    ```
    cd ~/Downloads \
    && gpg2 --verify Sphere_by_Horizen-1.3.0-beta.*.asc
    ```
    - Successful verification should produce a message like follows:
    ```
    gpg: assuming signed data in 'Sphere_by_Horizen-1.3.0-beta.{extension}'
    ...
    ```
### Verify File Integrity
1. Download Sphere_by_Horizen-1.3.0-beta.deb and Sphere_by_Horizen-1.3.0-beta.deb.sha256 and store the files in your Downloads folder for Debian based systems, or Sphere_by_Horizen-1.3.0-beta.AppImage and Sphere_by_Horizen-1.3.0-beta.AppImage.sha256 and store the files in your Downloads folder for all other Linux systems.
2. Open a Terminal window
3. Verify the checksums by copy/pasting the following command:
    ```
    cd ~/Downloads \
    && sha256sum -c Sphere_by_Horizen-1.3.0-beta.*.sha256
    
    ```
4. The output of the command should equal:
    ```
    Sphere_by_Horizen-1.3.0-beta.{extension}: OK
    ```
