...
1. Download Sphere_by_Horizen-1.21.0.exe and Sphere_by_Horizen-1.21.0.exe.asc and store the files in your Downloads folder.
...
1. Download Sphere_by_Horizen-1.21.0.exe and Sphere_by_Horizen-1.21.0.exe.sha256 and store the files in your Downloads folder
2. Open a PowerShell window
3. Verify the checksums by copy/pasting the following commands:
    ```
    cd ~\Downloads
    $targetHash = (Get-Content -Path Sphere_by_Horizen-1.21.0.exe.sha256 -Delimiter " ")[0].Trim()
    $fileName = (Get-Content -Path Sphere_by_Horizen-1.21.0.exe.sha256 -Delimiter " ")[2].Trim()
...
## MacOS
### Verify File Authenticity
1. Download Sphere_by_Horizen-1.21.0.dmg and Sphere_by_Horizen-1.21.0.dmg.asc and store the files in your Downloads folder.
...
### Verify File Integrity
1. Download Sphere_by_Horizen-1.21.0.dmg and Sphere_by_Horizen-1.21.0.dmg.sha256 and store the files in your Downloads folder
2. Open a Terminal window
3. Verify the checksums by copy/pasting the following command:
    ```
    cd ~/Downloads \
    && shasum -a256 -c Sphere_by_Horizen-1.21.0.dmg.sha256
...
## Linux
### Verify File Authenticity
1. Download Sphere_by_Horizen-1.21.0.deb and Sphere_by_Horizen-1.21.0.deb.asc and store the files in your Downloads folder for Debian based systems, or Sphere_by_Horizen-1.21.0.AppImage and Sphere_by_Horizen-1.21.0.AppImage.asc and store the files in your Downloads folder for all other Linux systems.
...
6. Verify the installer binary using the .asc signature (the .asc signature file must reside in the same directory as the installer binary):
    ```
    cd ~/Downloads \
    && gpg2 --verify Sphere_by_Horizen-1.21.0.*.asc
    ```
    - Successful verification should produce a message like follows:
    ```
    gpg: assuming signed data in 'Sphere_by_Horizen-1.21.0.{extension}'
...
3. Verify the checksums by copy/pasting the following command:
    ```
    cd ~/Downloads \
    && sha256sum -c Sphere_by_Horizen-1.21.0.*.sha256
